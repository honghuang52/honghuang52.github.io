...
class Index extends Common
{
    public function index()
    {
        ...
        $zxwenzhang = Db::name('posts')->where('post_type','in','0,2,3,4,5,6,7,8')->field('id,post_title')->order('post_modified desc')->limit(5)->cache(true, 300)->select();
        $this->assign('zxwenzhang', $zxwenzhang);
        $zxpinglun = Db::name('comments')->field('id,url,content')->order('createtime desc')->limit(5)->cache(true, 300)->select();
        ...
        $zxliuyan = Db::name('guestbook')->field('id,msg')->order('createtime desc')->limit(5)->cache(true, 300)->select();
    ...
    public function articles()
    {
        ...
        $this->assign('fenlei', $this->getfenlei());
        $this->assign('toutiao', intval(Operc::getc('toutiao')));
        return $this->view();
    }
    ...
    public function allpage()
    {
        ...
        $this->assign('zhuyemian', intval(Operc::getc('zhuyemian')));
        return $this->view();
    }
    ...
    public function addSlideshow()
    {
        ...
        if(Request::instance()->isPost() && $this->verification())
        {
            ...
            $sgroup = htmlspecialchars(Request::instance()->post('sgroup'));
            if(!empty($sgroup)){
                $slide = Operc::getc('slidegroups');
                if(!empty($slide)){
                    $slideArr = unserialize($slide);
                    $sid = $slideArr[$sgroup]['sid'];
                    if(empty($sid)){
                        $sid = $id;
                    }
                    else{
                        $sidarr = explode(',', $sid);
                        if(!in_array($id, $sidarr)){
                            $sidarr[] = $id;
                        }
                        $sid = implode(',', $sidarr);
                    }
                    $slideArr[$sgroup]['sid'] = $sid;
                    Operc::setc('slidegroups',serialize($slideArr));
                }
            }
        }
    ...
    public function modifyslide()
    {
        ...
        if(Request::instance()->isPost() && $this->verification())
        {
            ...
            $data = ['slide_name' => htmlspecialchars(Request::instance()->post('mingcheng')), 'slide_pic' => Request::instance()->post('slideshow'), 'slide_url' => htmlspecialchars(Request::instance()->post('lianjie')), 'slide_des' => htmlspecialchars(Request::instance()->post('miaoshu'))];
            $slid = Request::instance()->post('slideId');
            Db::name('slide')
                ->where('slide_id', $slid)
                ->update($data);
            $sgroup = htmlspecialchars(Request::instance()->post('sgroup'));
            ...
            if(!empty($slide)){
                ...
                foreach($slideArr as $key => $val){
                    if(strpos(','.$val['sid'].',', ','.$slid.',') !== false){
                        $tmpstr = str_replace(','.$slid.',', '', ','.$val['sid'].',');
                        $slideArr[$key]['sid'] = trim($tmpstr, ',');
                    }
                }
                $sid = $slideArr[$sgroup]['sid'];
                if(empty($sid)){
        ...
        $wgroup = '';
        foreach($slideArr as $key => $val){
            if(strpos(','.$val['sid'].',', ','.$wzid.',') !== false){
                $wgroup = $key;
            }
        }
        $this->assign('wgroup', $wgroup);
        $this->assign('backstageMenu', 'yemian');
    ...
    public function slideshow()
    {
        ...
        $slideGroup = '';
        if(Request::instance()->has('slideGroup','post')){
            $slideGroup = Request::instance()->post('slideGroup');
        }
        ...
        if(!empty($slide)){
            ...
            foreach($data as $key => $val){
                $data[$key]['group'] = $this->whichgroup($val['slide_id'], $slideArr);
            }
        }
        else{
            $this->assign('slide', []);
    ...
    public function uploadSlideshow()
    {
        ...
        if($this->verification()){
            ...
            $info = $file->move(ROOT_PATH . 'data' . DS . 'uploads');
            if($info){
                $slideshowWidth = intval(Request::instance()->post('width'));
                $slideshowHeight = intval(Request::instance()->post('height'));
                $image = \think\Image::open(ROOT_PATH . 'data' . DS . 'uploads' . DS . $info->getSaveName());
                @$image->thumb($slideshowWidth,$slideshowHeight,\think\Image::THUMB_FIXED)->save(ROOT_PATH . 'data' . DS . 'uploads' . DS . $info->getSaveName());
                echo str_replace('\\','/',$info->getSaveName());
            }else{
                echo $file->getError();
            }
    ...
    public function newslidegroup()
    {
        ...
        if(Request::instance()->isPost() && $this->verification())
        {
            $rule = [
                'fenzu' => 'require|alpha',
                'kuandu' => 'require|number',
                'gaodu' => 'require|number'
            ];
            $msg = [
                'fenzu.require' => Lang::get('Group name must be filled in'),
                'fenzu.alpha' => Lang::get('Group names can only be composed of letters'),
                'kuandu.require' => Lang::get('Slide width must be filled in'),
                'kuandu.number' => Lang::get('Slide width must be a number'),
                'gaodu.require' => Lang::get('Slide height must be filled in'),
                'gaodu.number' => Lang::get('Slide height must be a number')
            ];
            $data = [
                'fenzu' => htmlspecialchars(Request::instance()->post('fenzu')),
                'kuandu' => htmlspecialchars(Request::instance()->post('kuandu')),
                'gaodu' => htmlspecialchars(Request::instance()->post('gaodu'))
            ];
            $validate = new Validate($rule, $msg);
            if(!$validate->check($data))
            {
            ...
            if(!empty($slide)){
                $slideArr = unserialize($slide);
                if(isset($slideArr[$data['fenzu']])){
                    $this->error(Lang::get('The group name already exists, please change one'));
                    return false;
                }
            }
            $slideArr[$data['fenzu']] = [
        ...
        if(Request::instance()->has('paixu','post') && $this->verification())
        {
            ...
            if(count($slides) > 1){
                ...
                foreach($slides as $key => $val){
                    $slideArr[$key]['listorder'] = $val;
                    $slides[$key] = $slideArr[$key];
                }
            }
            Operc::setc('slidegroups', serialize($slides));
        }
        $slideArr = [];
        $slide = Operc::getc('slidegroups');
        if(!empty($slide)){
    ...
    public function modifyslidegroup()
    {
        ...
        if(Request::instance()->isPost() && $this->verification())
        {
            ...
            $slide = Operc::getc('slidegroups');
            $slideArr = unserialize($slide);
            $sname = htmlspecialchars(Request::instance()->post('slidegroupname'));
            if(!isset($slideArr[$sname])){
                $this->error(Lang::get('Your permission is insufficient'));
                return false;
            }
            $listorder = $slideArr[$sname]['listorder'];
            $sid = $slideArr[$sname]['sid'];
            $slideArr[$sname] = [
                'name' => $data['fenzu'],
                'wid' => $data['kuandu'],
                'hei' => $data['gaodu'],
            ...
            foreach($slideArr as $key => $val){
                if($key == $sname){
                    $tmpArr[$data['fenzu']] = $val;
                }
                else{
                    $tmpArr[$key] = $val;
                }
            }
            Operc::setc('slidegroups', serialize($tmpArr));
        }
        $slide = Operc::getc('slidegroups');
        $slideArr = unserialize($slide);
        $wzid = '';
        if(Request::instance()->has('fenzu','post'))
        {
            $wzid = Request::instance()->post('fenzu');
        }
        elseif(Request::instance()->has('c','get'))
        {
            $wzid = Request::instance()->get('c');
        }
        if(isset($slideArr[$wzid])){
            $this->assign('data', $slideArr[$wzid]);
        }
        else{
            $this->assign('data', []);
        }
    ...
    public function removeSlidegroup()
    {
        $this->checkPermissions(3);
        if($this->verification())
        {
            $slide = Operc::getc('slidegroups');
            $slideArr = unserialize($slide);
            $sname = htmlspecialchars(Request::instance()->post('name'));
            unset($slideArr[$sname]);
            Operc::setc('slidegroups', serialize($slideArr));
        }
        return true;
    }
    ...
    {
        Operc::setc('toutiao', Request::instance()->post('id'));
    ...
    {
        Operc::setc('zhuyemian', Request::instance()->post('id'));
